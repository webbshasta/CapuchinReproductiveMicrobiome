...
```{r}
library(phyloseq); library(vegan); library(ggplot2); library(decontam); library(ggpubr)
```
...
length(indToRemoveFromMetadata) #these are the samples that we need to remove from the metadata
```
...
```{r boxplots}
metadataStageFilt <- filter(metadataStageFilt, !is.na(ReproductiveStatus)) #pruning Fleur (she has no rep state)
psStage <- prune_samples(!sample_data(psStage)$INDIVIDUAL == "Fleur", psStage)
psStage #5012 taxa and 319 samples after removing Fleur

metadataStageFilt$ReproductiveStatus <- factor(metadataStageFilt$ReproductiveStatus, levels = c("Cycling", "Pregnant", "Nursing"))
