...
```{r}
seqtab.nochim <- readRDS("~/Desktop/PregnancyPaper2019/PregnancyMicrobiomeProject/seqtab.nochim.RDS"); rownames(seqtab.nochim)
...
```{r}
metadataStage <- read.csv("~/Desktop/PregnancyPaper2019/PregnancyMicrobiomeProject/MicrobiomeAnalysis/Metadata/repStateStageMetaData22April.csv")
as.data.frame(metadataStage)
```

```{r}
metadataStage <- metadataStage %>% filter(SEX != "Male"); View(metadataStage) #168 samples including controls
...
```{r data cleaning}
ControlsDF <- SSRmetadata %>% 
              filter(., SampleOrControl == "Control") %>% #adding controls to the metadata file for the rep stage analysis
              select(-X.2, -X.1) %>%
              mutate(StartDate = NA, EndDate = NA, RepStateStage = NA)

rownames(ControlsDF) <- ControlsDF$SAMPLE_ID #setting sampled ID to rownames
stageTest <- rbind(metadataStage, ControlsDF) #joining the controls back into the metadata
metadataStage <- stageTest #saving changes in metadata DF
```
